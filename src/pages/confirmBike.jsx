...
const confirmBike = () => {
  ...
  const bookCar = async () => {
    ...
    window.location.href = `/${encodedUserId}`;
    console.log(BookCar);
  }
  ...
  const fetchData = async () => {
    const getCarResponse = await axios.get(`http://localhost:8080/api/bikes/${ids}`);
    console.log(getCarResponse.data);

    const baseAmount = parseFloat(getCarResponse.data.bike.amount);
              ...
              <div class="paymentWidget w100">
                <div class="productWidget">
                  <div class="productImage">
                    <h2>{carData.bike.bikeName}</h2>
                    <figure>
                      <img src={"http://localhost:5173/@fs/D:/Sem_6/drive-ease-backend/assets/bikesImage/" + carData.bike.imageUrl} alt="" />
                    </figure>
                  </div>
